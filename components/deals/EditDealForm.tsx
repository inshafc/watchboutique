...
type WatchOption  = { id: string; watch_name: string; reference: string | null; status: string; purchase_cost: number | null; photos?: string[]; inventory_type?: string | null }
...
export default function EditDealForm({
  ...
  const [form, setForm] = useState({
    ...
    exchange_rate:     deal.exchange_rate?.toString() ?? '',
  })
  ...
  const [expenseRows,       setExpenseRows]       = useState<ExpenseRow[]>(
    initialExpenses.length > 0
      ? initialExpenses.map(e => ({ id: e.id, category: e.category as ExpenseCategory, custom_label: e.custom_label ?? '', amount: e.amount.toString() }))
      : [{ ...DEFAULT_EXPENSE }]
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await supabase.from('deal_expenses').delete().eq('deal_id', deal.id)
    if (otherCosts && expenseRows.length > 0) {
      const validRows = expenseRows.filter(r => num(r.amount) != null)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center gap-4 pt-2 pb-8">
        <button type="submit" disabled={loading} className="bg-sidebar text-white text-[13px] font-medium px-6 py-2.5 rounded-lg hover:bg-[#333] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
