...
export default async function DealsPage() {
  ...
  const [dealsRes, smRes, brandsRes] = await Promise.all([
    supabase
      .from('deals')
      .select('id, deal_type, stage, offered_price, sale_price, trade_value, adjustment, commission, payment_method, currency, exchange_rate, notes, sales_manager, closed_at, created_at, other_costs, other_costs_amount, commission_payable, commission_amount, new_client, source, sale_date, delivery_status, bank_name, cash_amount, bank_amount, deleted_at, watch_id, client_id, watches(watch_name, reference, status, photos, purchase_cost, sold_price, brand_id, brands(id, name, color)), clients(name, avatar_color, is_vip, club_twb)')
      .is('deleted_at', null)
