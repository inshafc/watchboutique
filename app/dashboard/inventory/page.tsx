...
export default async function InventoryPage({
  ...
  const [watchesRes, brandsRes] = await Promise.all([
    supabase.from('watches').select('id, watch_name, reference, serial_number, date_on_card, date_acquired, condition, set_details, purchased_from, purchase_cost, selling_price, sold_price, currency, status, watch_status, comments, photos, labels, is_draft, deleted_at, created_at, brand_id, inventory_type, consignee_name, brands(name, color)').is('deleted_at', null).order('created_at', { ascending: false }),
